...
exports.viewProfessionals = async(req,res)=>{
  ...
  const province = req.body.province || req.user[0].province;
  const city = req.body.city || req.user[0].city;
  const zone = req.body.zone || req.user[0].zone;
  
  let searchOptions = {
    where: { jobTitle, isVerified: true },
    attributes: { exclude: ['password', 'role', 'isVerified', 'otp', 'isOtpVerified', 'createdAt', 'updatedAt'] }
  }
  ...
  if (Employees.length ==0){ 
   ...
   Employees.sort((a, b) => {
    ...
    const aMatch = a.province === province && a.city === city && a.zone === zone;
    const bMatch = b.province === province && b.city === city && b.zone === zone;

    // Check if a and b match province and city only
    const aProvinceCityMatch = a.province === province && a.city === city && a.zone !== zone;
    const bProvinceCityMatch = b.province === province && b.city === city && b.zone !== zone;

    // Check if a and b match province only
    const aProvinceMatch = a.province === province && a.city !== city && a.zone !== zone;
    const bProvinceMatch = b.province === province && b.city !== city && b.zone !== zone;
